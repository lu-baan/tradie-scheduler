...
import { useListJobs, ListJobsSortBy } from "@workspace/api-client-react";
import { JobCard } from "@/components/jobs/JobCard";
import { JobForm } from "@/components/jobs/JobForm";
import { useGeolocation } from "@/hooks/use-geolocation";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";
import { Plus, SlidersHorizontal, MapPin, Loader2 } from "lucide-react";
...
export function JobsList() {
  ...
  // Client-side filtering for tabs (since API doesn't have an exact multi-status filter in one param, we filter here for UI speed)
  const filteredJobs = (jobs || []).filter(job => {
    if (filterType === "all") return job.status !== "completed";
    if (filterType === "completed") return job.status === "completed";
    return job.jobType === filterType && job.status !== "completed";
  });
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-card border border-white/5 rounded-xl p-4 shadow-xl">
        <Tabs.Root value={filterType} onValueChange={(v) => setFilterType(v as any)}>
          ...
          <Tabs.Content value={filterType} className="outline-none">
            {isLoading ? (
